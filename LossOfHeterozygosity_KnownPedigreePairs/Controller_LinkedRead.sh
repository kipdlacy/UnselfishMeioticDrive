...
python ../AlignmentAndVariantCallingWGS/Process_VCFtoPosScreener.py  ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs

# Use vcftools to create a new VCF file that contains only those high quality, trusted positions
vcftools \
	--vcf ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf \
	--positions ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.PosScreener \
	--recode \
	--recode-INFO-all \
	--out ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs
...
for COMP in $Comparisons; do
	...
	$GATK SelectVariants \
		-V ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode.vcf \
		--sample-name $Sample1 \
		--sample-name $Sample2 \
		--exclude-non-variants \
		--remove-unused-alternates \
		-O ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode_$COMP'.vcf'
	#Write variants to table
	$GATK VariantsToTable \
		-V ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode_$COMP'.vcf' \
		-F CHROM -F POS -F REF -F ALT -F FILTER \
		-GF GT \
		-O ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode_$COMP'.vcf.table'
	...
	python Process_GetDiffs.py ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode_$COMP'.vcf''.table'
done
...
$GATK VariantsToTable \
	...
	-F CHROM -F POS -F REF -F ALT -F FILTER \
	-GF GT \
	-O ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode.vcf.table

# Writing one file for each individual that contains all sites at which each individual is heterozygous
python WriteIndividualSampleHetFiles.py ./data/LinkedRead_KPpairs_SNPs_GATKfilt_RmFilt_NoHapHet.recode.vcf.table_PutAncHet_ADDPscreenNucs.recode.vcf.table
